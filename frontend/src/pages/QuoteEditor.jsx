...
export default function QuoteEditor(){
  ...
  const [dayManagerModalOpen, setDayManagerModalOpen] = useState(false);
...
const CATEGORY_GROUPS = {
  ...
  // --- Day Manager Modal handlers ---
  // Handle day deletion from modal: move services to trash
  const handleDayManagerDelete = useCallback((dayToDelete) => {
    if (!dayToDelete?.lines || dayToDelete.lines.length === 0) return;
    
    // Move all services from the deleted day to trashLines
    const servicesToTrash = dayToDelete.lines.map(line => ({
      ...line,
      deleted: true,
      dayId: null, // Remove day association
      originalDayId: dayToDelete.id // Keep reference for potential restore
    }));
    
    setTrashLines(prev => [...prev, ...servicesToTrash]);
  }, []);

  // Apply changes from Day Manager Modal
  const handleDayManagerApply = useCallback((newDays) => {
    if (!newDays || newDays.length === 0) {
      showNotice("At least 1 day is required", "info");
      return;
    }

    // Normalize positions
    const normalized = normalizeQuotePositions({ ...q, days: newDays });
    
    // Recompute start/end dates if we have dates
    const daysWithDates = normalized.days.filter(d => d.date);
    let nextStart = q.start_date || null;
    let nextEnd = q.end_date || null;
    
    if (daysWithDates.length > 0) {
      const sortedDates = daysWithDates.map(d => d.date).sort();
      nextStart = sortedDates[0];
      nextEnd = sortedDates[sortedDates.length - 1];
    }
    
    // Update quote with new days order
    setQ(prev => ({
      ...prev,
    ...
    // Update active day if it still exists, otherwise select first day
    const activeDayStillExists = normalized.days.some(d => d.id === activeDayId);
    if (!activeDayStillExists && normalized.days.length > 0) {
      setActiveDayId(normalized.days[0].id);
    }
    
    showNotice("Itinerary updated", "success");
  }, [q, activeDayId, showNotice]);
  ...
  return (
    ...
    <div className="app">
          ...
          <button 
            ...
            onClick={async () => {
              ...
              try {
                ...
                showNotice(`Export failed: ${errorMsg}`, "error");
              }
            }}
        ...
        <div className="rail">
          <div className="left-list">
            ...
            <div className="left-group">
              {/* Edit itinerary days button */}
              <button
                className="day-pill"
                onClick={() => setDayManagerModalOpen(true)}
      ...
      {/* Day Manager Modal */}
      <DayManagerModal
        open={dayManagerModalOpen}
        days={q?.days || []}
        onClose={() => setDayManagerModalOpen(false)}
        onApply={handleDayManagerApply}
        onDeleteDay={handleDayManagerDelete}
      />
